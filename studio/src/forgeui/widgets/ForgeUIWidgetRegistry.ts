...
import { FORGEUI_POWER_FLOW_CARD_DEFAULT_SIZE } from '../ForgeUIPowerFlowCard'
...
const categories: Record<ForgeUIWidgetCategory, ComponentType[]> = {
  ...
  Dashboard: ['DashboardCard', 'SensorTile', 'RelayPanel', 'PwmController', 'Chart', 'TrendChartPro', 'AlarmPanel', 'IOMonitor', 'BatteryCard', 'TankLevelCard', 'NetworkStatusCard', 'DeviceSummaryCard', 'KpiCard', 'PowerFlowCard'],
  Assets: [
    'InteractiveButton',
    'InteractiveLight',
...
const sizes: Partial<Record<ComponentType, [number, number]>> = {
  ...
  AlarmPanel: [420, 300],
  IOMonitor: [420, 300],
  BatteryCard: [FORGEUI_BATTERY_CARD_DEFAULT_SIZE.width, FORGEUI_BATTERY_CARD_DEFAULT_SIZE.height],
  ...
  PowerFlowCard: [FORGEUI_POWER_FLOW_CARD_DEFAULT_SIZE.width, FORGEUI_POWER_FLOW_CARD_DEFAULT_SIZE.height],
}
...
const keywords: Partial<Record<ComponentType, string[]>> = {
  ...
  PowerFlowCard: ['power', 'flow', 'grid', 'solar', 'battery', 'load', 'energy', 'dashboard', 'forgeui native'],
}
...
const describe = (
  ...
  const special: Partial<Record<ComponentType, string>> = {
    ...
    PowerFlowCard: 'ForgeUI Native compact four-node electrical power flow monitor.',
...
export const forgeUIWidgetDefinitions: ForgeUIWidgetDefinition[] =
  (Object.values(categories).flat() as ComponentType[]).map(type => {
    ...
    return {
      ...
      status: 'available',
      origin: type === 'DashboardCard' || type === 'SensorTile' || type === 'RelayPanel' || type === 'PwmController' || type === 'Chart' || type === 'TrendChartPro' || type === 'AlarmPanel' || type === 'IOMonitor' || type === 'BatteryCard' || type === 'TankLevelCard' || type === 'NetworkStatusCard' || type === 'DeviceSummaryCard' || type === 'KpiCard' || type === 'PowerFlowCard' ? 'forgeui-native' : 'lvgl-standard',
      ...(type === 'DashboardCard' || type === 'SensorTile' || type === 'RelayPanel' || type === 'PwmController' || type === 'Chart' || type === 'TrendChartPro' || type === 'AlarmPanel' || type === 'IOMonitor' || type === 'BatteryCard' || type === 'TankLevelCard' || type === 'NetworkStatusCard' || type === 'DeviceSummaryCard' || type === 'KpiCard' || type === 'PowerFlowCard' ? { nativeWidgetSchemaVersion: 1 } : {}),
      ...(type === 'DashboardCard' || type === 'SensorTile' || type === 'RelayPanel' || type === 'PwmController' || type === 'Chart' || type === 'TrendChartPro' || type === 'AlarmPanel' || type === 'IOMonitor' || type === 'BatteryCard' || type === 'TankLevelCard' || type === 'NetworkStatusCard' || type === 'DeviceSummaryCard' || type === 'KpiCard' || type === 'PowerFlowCard' ? { platform: {
