...
import { normalizeForgeUITankLevelCard, simulateForgeUITank } from './ForgeUITankLevelCard'
import { normalizeForgeUINetworkStatusCard, simulateForgeUINetworkStatus } from './ForgeUINetworkStatusCard'
import { normalizeForgeUIDeviceSummaryCard, simulateForgeUIDeviceSummary } from './ForgeUIDeviceSummaryCard'
import { normalizeForgeUIKpiCard, simulateForgeUIKpi } from './ForgeUIKpiCard'
import { normalizeForgeUIPowerFlowCard, simulateForgeUIPowerFlow } from './ForgeUIPowerFlowCard'
import {
...
type TankCardExport={stem:string;rootName:string;labelName:string;barName:string;runtimeEnabled:boolean;setLevel:string;setVolume:string;setCapacity:string;setState:string;setFlow:string;setThresholds:string;setStatus:string;selected?:string;low?:string;critical?:string;full?:string;overflow?:string;click?:string}
const createTankCardExports=(components:IComponents,used:Set<string>,events:Set<string>)=>{const out=new Map<string,TankCardExport>();const stems=new Set<string>();Object.values(components).filter(c=>c.type==='TankLevelCard').sort((a,b)=>a.id.localeCompare(b.id)).forEach(c=>{const base=toCIdentifier(c.id,'Tank').replace(/([a-z0-9])([A-Z])/g,'$1_$2');let stem=base,n=2;while(stems.has(stem))stem=`${base}_${n++}`;stems.add(stem);const r=stem.toLowerCase(),m=normalizeForgeUITankLevelCard(c.props);const hook=(suffix:string)=>{let h=`FG_On_${stem}_${suffix}`,i=2;while(used.has(h))h=`FG_On_${stem}_${suffix}_${i++}`;used.add(h);events.add(h);return h};const selected=m.enableUserEvents?hook('Selected'):undefined;out.set(c.id,{stem,rootName:`fg_${r}_tank`,labelName:`fg_${r}_tank_text`,barName:`fg_${r}_tank_level`,runtimeEnabled:m.generateRuntimeApi,setLevel:`FG_Set_${stem}_Level`,setVolume:`FG_Set_${stem}_Volume`,setCapacity:`FG_Set_${stem}_Capacity`,setState:`FG_Set_${stem}_State`,setFlow:`FG_Set_${stem}_Flow`,setThresholds:`FG_Set_${stem}_Thresholds`,setStatus:`FG_Set_${stem}_Status`,selected,low:m.enableUserEvents?hook('Low'):undefined,critical:m.enableUserEvents?hook('Critical'):undefined,full:m.enableUserEvents?hook('Full'):undefined,overflow:m.enableUserEvents?hook('Overflow'):undefined,click:selected?`fg_${r}_tank_clicked_cb`:undefined})});return out}

type NetworkCardExport={stem:string;rootName:string;labelName:string;runtimeEnabled:boolean;setState:string;setSsid:string;setIp:string;setGateway:string;setRssi:string;setLatency:string;setCloud:string;setMqtt:string;setInternet:string;setLocalApi:string;setStatus:string;selected?:string;reconnect?:string;details?:string;click?:string}
const createNetworkCardExports=(components:IComponents,used:Set<string>,events:Set<string>)=>{const out=new Map<string,NetworkCardExport>(),stems=new Set<string>();Object.values(components).filter(c=>c.type==='NetworkStatusCard').sort((a,b)=>a.id.localeCompare(b.id)).forEach(c=>{const base=toCIdentifier(c.id,'Network').replace(/([a-z0-9])([A-Z])/g,'$1_$2');let stem=base,n=2;while(stems.has(stem))stem=`${base}_${n++}`;stems.add(stem);const r=stem.toLowerCase(),m=normalizeForgeUINetworkStatusCard(c.props);const hook=(suffix:string)=>{let h=`FG_On_${stem}_Network_${suffix}`,i=2;while(used.has(h))h=`FG_On_${stem}_Network_${suffix}_${i++}`;used.add(h);events.add(h);return h};const selected=m.enableUserEvents?hook('Selected'):undefined;out.set(c.id,{stem,rootName:`fg_${r}_network`,labelName:`fg_${r}_network_text`,runtimeEnabled:m.generateRuntimeApi,setState:`FG_Set_${stem}_Network_State`,setSsid:`FG_Set_${stem}_Network_SSID`,setIp:`FG_Set_${stem}_Network_IP`,setGateway:`FG_Set_${stem}_Network_Gateway`,setRssi:`FG_Set_${stem}_Network_RSSI`,setLatency:`FG_Set_${stem}_Network_Latency`,setCloud:`FG_Set_${stem}_Network_Cloud`,setMqtt:`FG_Set_${stem}_Network_MQTT`,setInternet:`FG_Set_${stem}_Network_Internet`,setLocalApi:`FG_Set_${stem}_Network_Local_API`,setStatus:`FG_Set_${stem}_Network_Status`,selected,reconnect:m.enableUserEvents?hook('Reconnect_Requested'):undefined,details:m.enableUserEvents?hook('Details_Requested'):undefined,click:selected?`fg_${r}_network_clicked_cb`:undefined})});return out}

type DeviceSummaryExport={stem:string;rootName:string;labelName:string;runtimeEnabled:boolean;setName:string;setType:string;setModel:string;setFirmware:string;setBuild:string;setHealth:string;setUptime:string;setNetwork:string;setIp:string;setStorage:string;setAlarms:string;setPower:string;setBattery:string;setMaintenance:string;setStatus:string;selected?:string;details?:string;maintenance?:string;alarms?:string;click?:string;detailsClick?:string;maintenanceClick?:string;alarmsClick?:string}
const createDeviceSummaryExports=(components:IComponents,used:Set<string>,events:Set<string>)=>{const out=new Map<string,DeviceSummaryExport>(),stems=new Set<string>();Object.values(components).filter(c=>c.type==='DeviceSummaryCard').sort((a,b)=>a.id.localeCompare(b.id)).forEach(c=>{const base=toCIdentifier(c.id,'Device').replace(/([a-z0-9])([A-Z])/g,'$1_$2');let stem=base,n=2;while(stems.has(stem))stem=`${base}_${n++}`;stems.add(stem);const r=stem.toLowerCase(),m=normalizeForgeUIDeviceSummaryCard(c.props);const hook=(suffix:string)=>{let h=`FG_On_${stem}_Device_${suffix}`,i=2;while(used.has(h))h=`FG_On_${stem}_Device_${suffix}_${i++}`;used.add(h);events.add(h);return h};const selected=m.enableUserEvents&&m.clickable?hook('Selected'):undefined,details=m.enableUserEvents?hook('Details_Requested'):undefined,maintenance=m.enableUserEvents?hook('Maintenance_Requested'):undefined,alarms=m.enableUserEvents?hook('Alarms_Requested'):undefined;out.set(c.id,{stem,rootName:`fg_${r}_device`,labelName:`fg_${r}_device_text`,runtimeEnabled:m.generateRuntimeApi,setName:`FG_Set_${stem}_Device_Name`,setType:`FG_Set_${stem}_Device_Type`,setModel:`FG_Set_${stem}_Device_Model`,setFirmware:`FG_Set_${stem}_Device_Firmware`,setBuild:`FG_Set_${stem}_Device_Build`,setHealth:`FG_Set_${stem}_Device_Health`,setUptime:`FG_Set_${stem}_Device_Uptime_Seconds`,setNetwork:`FG_Set_${stem}_Device_Network_State`,setIp:`FG_Set_${stem}_Device_IP`,setStorage:`FG_Set_${stem}_Device_Storage`,setAlarms:`FG_Set_${stem}_Device_Alarm_Counts`,setPower:`FG_Set_${stem}_Device_Power_Source`,setBattery:`FG_Set_${stem}_Device_Battery_Percentage`,setMaintenance:`FG_Set_${stem}_Device_Maintenance`,setStatus:`FG_Set_${stem}_Device_Status`,selected,details,maintenance,alarms,click:selected?`fg_${r}_device_clicked_cb`:undefined,detailsClick:details?`fg_${r}_device_details_cb`:undefined,maintenanceClick:maintenance?`fg_${r}_device_maintenance_cb`:undefined,alarmsClick:alarms?`fg_${r}_device_alarms_cb`:undefined})});return out}

type KpiCardExport={stem:string;rootName:string;labelName:string;barName:string;runtimeEnabled:boolean;setValue:string;setUnits:string;setStatus:string;setColour:string;setProgress:string;setDelta:string;setTarget:string;setUpdated:string;selected?:string;details?:string;click?:string;detailsClick?:string}
const createKpiCardExports=(components:IComponents,used:Set<string>,events:Set<string>)=>{const out=new Map<string,KpiCardExport>(),stems=new Set<string>();Object.values(components).filter(c=>c.type==='KpiCard').sort((a,b)=>a.id.localeCompare(b.id)).forEach(c=>{const base=toCIdentifier(c.id,'KPI').replace(/([a-z0-9])([A-Z])/g,'$1_$2');let stem=base,n=2;while(stems.has(stem))stem=`${base}_${n++}`;stems.add(stem);const r=stem.toLowerCase(),m=normalizeForgeUIKpiCard(c.props);const hook=(suffix:string)=>{let h=`FG_On_${stem}_${suffix}`,i=2;while(used.has(h))h=`FG_On_${stem}_${suffix}_${i++}`;used.add(h);events.add(h);return h};const selected=m.enableUserEvents&&m.clickable?hook('Selected'):undefined,details=m.enableUserEvents?hook('Details_Requested'):undefined;out.set(c.id,{stem,rootName:`fg_${r}_kpi`,labelName:`fg_${r}_kpi_text`,barName:`fg_${r}_kpi_progress`,runtimeEnabled:m.generateRuntimeApi,setValue:`FG_Set_${stem}_Value`,setUnits:`FG_Set_${stem}_Units`,setStatus:`FG_Set_${stem}_Status`,setColour:`FG_Set_${stem}_Status_Colour`,setProgress:`FG_Set_${stem}_Progress`,setDelta:`FG_Set_${stem}_Delta`,setTarget:`FG_Set_${stem}_Target`,setUpdated:`FG_Set_${stem}_Updated_Text`,selected,details,click:selected?`fg_${r}_kpi_clicked_cb`:undefined,detailsClick:details?`fg_${r}_kpi_details_cb`:undefined})});return out}

type PowerFlowExport={stem:string;rootName:string;labelName:string;runtimeEnabled:boolean;setSolar:string;setLoad:string;setBattery:string;setGrid:string;setGenerator:string;setEfficiency:string;setEnergy:string;setState:string;setOnline:string;setFault:string;setStatus:string;setNode:string;setConnection:string;selected?:string;nodeSelected?:string;details?:string;control?:string;click?:string}
const createPowerFlowExports=(components:IComponents,used:Set<string>,events:Set<string>)=>{const out=new Map<string,PowerFlowExport>(),stems=new Set<string>();Object.values(components).filter(c=>c.type==='PowerFlowCard').sort((a,b)=>a.id.localeCompare(b.id)).forEach(c=>{const base=toCIdentifier(c.id,'Power_Flow').replace(/([a-z0-9])([A-Z])/g,'$1_$2');let stem=base,n=2;while(stems.has(stem))stem=`${base}_${n++}`;stems.add(stem);const r=stem.toLowerCase(),m=normalizeForgeUIPowerFlowCard(c.props);const hook=(suffix:string)=>{let h=`FG_On_${stem}_Power_Flow_${suffix}`,i=2;while(used.has(h))h=`FG_On_${stem}_Power_Flow_${suffix}_${i++}`;used.add(h);events.add(h);return h};const selected=m.enableUserEvents&&m.clickable?hook('Selected'):undefined;out.set(c.id,{stem,rootName:`fg_${r}_power_flow`,labelName:`fg_${r}_power_flow_text`,runtimeEnabled:m.generateRuntimeApi,setSolar:`FG_Set_${stem}_Power_Flow_Solar_Watts`,setLoad:`FG_Set_${stem}_Power_Flow_Load_Watts`,setBattery:`FG_Set_${stem}_Power_Flow_Battery_Watts`,setGrid:`FG_Set_${stem}_Power_Flow_Grid_Watts`,setGenerator:`FG_Set_${stem}_Power_Flow_Generator_Watts`,setEfficiency:`FG_Set_${stem}_Power_Flow_Efficiency`,setEnergy:`FG_Set_${stem}_Power_Flow_Energy_Today`,setState:`FG_Set_${stem}_Power_Flow_State`,setOnline:`FG_Set_${stem}_Power_Flow_Node_Online`,setFault:`FG_Set_${stem}_Power_Flow_Node_Fault`,setStatus:`FG_Set_${stem}_Power_Flow_Status`,setNode:`FG_Set_${stem}_Power_Flow_Node_Value`,setConnection:`FG_Set_${stem}_Power_Flow_Connection`,selected,nodeSelected:m.enableUserEvents?hook('Node_Selected'):undefined,details:m.enableUserEvents?hook('Details_Requested'):undefined,control:m.enableUserEvents&&m.enableControlRequests?hook('Control_Requested'):undefined,click:selected?`fg_${r}_power_flow_clicked_cb`:undefined})});return out}
...
const buildLvglBlock = (
  ...
  tankCardExports: Map<string, TankCardExport>,
  networkCardExports: Map<string, NetworkCardExport>,
  deviceSummaryExports: Map<string, DeviceSummaryExport>,
  kpiCardExports: Map<string, KpiCardExport>,
  powerFlowExports: Map<string, PowerFlowExport>,
) => {
...
case 'TankLevelCard':{const m=simulateForgeUITank(normalizeForgeUITankLevelCard(child.props)),t=tankCardExports.get(child.id);if(!t)break;const d=`${t.rootName}_data`;lines.push(`${t.rootName}=lv_obj_create(${parentVar});lv_obj_set_pos(${t.rootName},${x},${y});lv_obj_set_size(${t.rootName},${w},${h});lv_obj_set_style_bg_color(${t.rootName},lv_color_hex(${palette.surface}),LV_PART_MAIN);lv_obj_set_style_border_color(${t.rootName},lv_color_hex(${palette.surfaceBorder}),LV_PART_MAIN);lv_obj_set_style_radius(${t.rootName},${m.rounded?12:0},LV_PART_MAIN);lv_obj_clear_flag(${t.rootName},LV_OBJ_FLAG_SCROLLABLE);`);lines.push(`lv_obj_t*${varName}_title=lv_label_create(${t.rootName});lv_label_set_text(${varName}_title,"${esc(m.title)}");lv_obj_align(${varName}_title,LV_ALIGN_TOP_LEFT,0,0);${t.barName}=lv_bar_create(${t.rootName});lv_obj_set_pos(${t.barName},0,32);lv_obj_set_size(${t.barName},72,${Math.max(80,Number(h)-70||220)});lv_bar_set_range(${t.barName},0,100);${t.labelName}=lv_label_create(${t.rootName});lv_obj_set_pos(${t.labelName},90,38);`);lines.push(`${d}.percentage=${cFloatLiteral(m.percentage)};${d}.volume=${cFloatLiteral(m.volume)};${d}.capacity=${cFloatLiteral(m.capacity)};${d}.flow=${cFloatLiteral(m.flowRate)};${d}.low=${cFloatLiteral(m.lowThreshold)};${d}.critical=${cFloatLiteral(m.criticalThreshold)};${d}.full=${cFloatLiteral(m.fullThreshold)};${d}.overflow=${cFloatLiteral(m.overflowThreshold)};${d}.state=FG_TANK_STATE_${m.state.toUpperCase()};snprintf(${d}.status,sizeof(${d}.status),"%s","${escPrintfLiteral(m.statusText)}");${t.rootName}_render();`);if(t.click)lines.push(`lv_obj_add_flag(${t.rootName},LV_OBJ_FLAG_CLICKABLE);lv_obj_add_event_cb(${t.rootName},${t.click},LV_EVENT_CLICKED,NULL);`);lines.push(``);break}

case 'NetworkStatusCard':{const m=simulateForgeUINetworkStatus(normalizeForgeUINetworkStatusCard(child.props)),n=networkCardExports.get(child.id);if(!n)break;const d=`${n.rootName}_data`,state=m.state.replace(/-/g,'_').toUpperCase();lines.push(`${n.rootName}=lv_obj_create(${parentVar});lv_obj_set_pos(${n.rootName},${x},${y});lv_obj_set_size(${n.rootName},${w},${h});lv_obj_set_style_bg_color(${n.rootName},lv_color_hex(${palette.surface}),LV_PART_MAIN);lv_obj_set_style_border_color(${n.rootName},lv_color_hex(${palette.surfaceBorder}),LV_PART_MAIN);lv_obj_set_style_radius(${n.rootName},${m.rounded?12:0},LV_PART_MAIN);lv_obj_clear_flag(${n.rootName},LV_OBJ_FLAG_SCROLLABLE);lv_obj_t*${varName}_title=lv_label_create(${n.rootName});lv_label_set_text(${varName}_title,"${esc(m.title)}");lv_obj_align(${varName}_title,LV_ALIGN_TOP_LEFT,0,0);${n.labelName}=lv_label_create(${n.rootName});lv_obj_set_pos(${n.labelName},0,32);lv_obj_set_size(${n.labelName},${Math.max(100,Number(w)-28||340)},${Math.max(80,Number(h)-58||240)});`);lines.push(`${d}.state=FG_NETWORK_STATE_${state};${d}.rssi=${Math.round(m.rssi)};${d}.latency=${Math.round(m.latency)};${d}.cloud=${m.cloudConnected};${d}.mqtt=${m.mqttConnected};${d}.internet=${m.internetAvailable};${d}.local_api=${m.localApiAvailable};snprintf(${d}.ssid,sizeof(${d}.ssid),"%s","${escPrintfLiteral(m.ssid)}");snprintf(${d}.ip,sizeof(${d}.ip),"%s","${escPrintfLiteral(m.ipAddress)}");snprintf(${d}.gateway,sizeof(${d}.gateway),"%s","${escPrintfLiteral(m.gateway)}");snprintf(${d}.status,sizeof(${d}.status),"%s","${escPrintfLiteral(m.statusText)}");${n.rootName}_render();`);if(n.click)lines.push(`lv_obj_add_flag(${n.rootName},LV_OBJ_FLAG_CLICKABLE);lv_obj_add_event_cb(${n.rootName},${n.click},LV_EVENT_CLICKED,NULL);`);lines.push(``);break}

case 'DeviceSummaryCard':{const m=simulateForgeUIDeviceSummary(normalizeForgeUIDeviceSummaryCard(child.props)),s=deviceSummaryExports.get(child.id);if(!s)break;const d=`${s.rootName}_data`,power=m.powerSource.replace(/-/g,'_').toUpperCase(),network=m.networkState.replace(/-/g,'_').toUpperCase();lines.push(`${s.rootName}=lv_obj_create(${parentVar});lv_obj_set_pos(${s.rootName},${x},${y});lv_obj_set_size(${s.rootName},${w},${h});lv_obj_set_style_bg_color(${s.rootName},lv_color_hex(${palette.surface}),LV_PART_MAIN);lv_obj_set_style_border_color(${s.rootName},lv_color_hex(${palette.surfaceBorder}),LV_PART_MAIN);lv_obj_set_style_radius(${s.rootName},${m.rounded?12:0},LV_PART_MAIN);lv_obj_clear_flag(${s.rootName},LV_OBJ_FLAG_SCROLLABLE);lv_obj_t*${varName}_title=lv_label_create(${s.rootName});lv_label_set_text(${varName}_title,"${esc(m.title)}");lv_obj_align(${varName}_title,LV_ALIGN_TOP_LEFT,0,0);${s.labelName}=lv_label_create(${s.rootName});lv_obj_set_pos(${s.labelName},0,32);lv_obj_set_size(${s.labelName},${Math.max(100,Number(w)-28||360)},${Math.max(80,Number(h)-96||202)});`);lines.push(`${d}.health=FG_DEVICE_HEALTH_${m.health.toUpperCase()};${d}.power=FG_DEVICE_POWER_${power};${d}.network=FG_NETWORK_STATE_${network};${d}.uptime=${Math.round(m.uptimeSeconds)}u;${d}.active=${Math.round(m.activeAlarmCount)}u;${d}.critical=${Math.round(m.criticalAlarmCount)}u;${d}.storage=${m.storageAvailable};${d}.storage_free=${Math.round(m.storageFree)}ULL;${d}.battery=${cFloatLiteral(m.batteryPercentage)};${d}.maintenance=${m.maintenanceRequired};snprintf(${d}.name,sizeof(${d}.name),"%s","${escPrintfLiteral(m.deviceName)}");snprintf(${d}.type,sizeof(${d}.type),"%s","${escPrintfLiteral(m.deviceType)}");snprintf(${d}.model,sizeof(${d}.model),"%s","${escPrintfLiteral(m.model)}");snprintf(${d}.firmware,sizeof(${d}.firmware),"%s","${escPrintfLiteral(m.firmwareVersion)}");snprintf(${d}.build,sizeof(${d}.build),"%s","${escPrintfLiteral(m.buildVersion)}");snprintf(${d}.ip,sizeof(${d}.ip),"%s","${escPrintfLiteral(m.ipAddress)}");snprintf(${d}.maintenance_text,sizeof(${d}.maintenance_text),"%s","${escPrintfLiteral(m.maintenanceMessage)}");snprintf(${d}.status,sizeof(${d}.status),"%s","${escPrintfLiteral(m.statusText)}");${s.rootName}_render();`);if(s.click)lines.push(`lv_obj_add_flag(${s.rootName},LV_OBJ_FLAG_CLICKABLE);lv_obj_add_event_cb(${s.rootName},${s.click},LV_EVENT_CLICKED,NULL);`);if(s.detailsClick)lines.push(`lv_obj_t*${varName}_details=lv_btn_create(${s.rootName});lv_obj_set_pos(${varName}_details,0,${Math.max(80,Number(h)-58||242)});lv_obj_set_size(${varName}_details,90,34);lv_obj_add_event_cb(${varName}_details,${s.detailsClick},LV_EVENT_CLICKED,NULL);lv_obj_t*${varName}_details_label=lv_label_create(${varName}_details);lv_label_set_text(${varName}_details_label,"Details");lv_obj_center(${varName}_details_label);`);if(s.alarmsClick)lines.push(`lv_obj_t*${varName}_alarms=lv_btn_create(${s.rootName});lv_obj_set_pos(${varName}_alarms,98,${Math.max(80,Number(h)-58||242)});lv_obj_set_size(${varName}_alarms,90,34);lv_obj_add_event_cb(${varName}_alarms,${s.alarmsClick},LV_EVENT_CLICKED,NULL);lv_obj_t*${varName}_alarms_label=lv_label_create(${varName}_alarms);lv_label_set_text(${varName}_alarms_label,"Alarms");lv_obj_center(${varName}_alarms_label);`);if(s.maintenanceClick)lines.push(`lv_obj_t*${varName}_maintenance=lv_btn_create(${s.rootName});lv_obj_set_pos(${varName}_maintenance,196,${Math.max(80,Number(h)-58||242)});lv_obj_set_size(${varName}_maintenance,120,34);lv_obj_add_event_cb(${varName}_maintenance,${s.maintenanceClick},LV_EVENT_CLICKED,NULL);lv_obj_t*${varName}_maintenance_label=lv_label_create(${varName}_maintenance);lv_label_set_text(${varName}_maintenance_label,"Maintenance");lv_obj_center(${varName}_maintenance_label);`);lines.push(``);break}

case 'KpiCard':{const m=simulateForgeUIKpi(normalizeForgeUIKpiCard(child.props)),k=kpiCardExports.get(child.id);if(!k)break;const d=`${k.rootName}_data`;lines.push(`${k.rootName}=lv_obj_create(${parentVar});lv_obj_set_pos(${k.rootName},${x},${y});lv_obj_set_size(${k.rootName},${w},${h});lv_obj_set_style_bg_color(${k.rootName},lv_color_hex(${palette.surface}),LV_PART_MAIN);lv_obj_set_style_border_color(${k.rootName},lv_color_hex(${palette.surfaceBorder}),LV_PART_MAIN);lv_obj_set_style_radius(${k.rootName},${m.rounded?12:0},LV_PART_MAIN);lv_obj_clear_flag(${k.rootName},LV_OBJ_FLAG_SCROLLABLE);lv_obj_t*${varName}_title=lv_label_create(${k.rootName});lv_label_set_text(${varName}_title,"${esc(m.title)}");lv_obj_align(${varName}_title,LV_ALIGN_TOP_LEFT,0,0);${k.labelName}=lv_label_create(${k.rootName});lv_obj_set_pos(${k.labelName},0,34);${k.barName}=lv_bar_create(${k.rootName});lv_obj_set_pos(${k.barName},0,${Math.max(90,Number(h)-58||162)});lv_obj_set_size(${k.barName},${Math.max(100,Number(w)-28||260)},12);lv_bar_set_range(${k.barName},0,100);`);lines.push(`${d}.value=${cFloatLiteral(m.value)};${d}.progress=${cFloatLiteral(m.progress)};${d}.delta=${cFloatLiteral(m.delta)};${d}.target=${cFloatLiteral(m.target)};${d}.colour=0x${Math.round(m.statusColour).toString(16)}u;snprintf(${d}.units,sizeof(${d}.units),"%s","${escPrintfLiteral(m.units)}");snprintf(${d}.status,sizeof(${d}.status),"%s","${escPrintfLiteral(m.status)}");snprintf(${d}.updated,sizeof(${d}.updated),"%s","${escPrintfLiteral(m.updatedText)}");${k.rootName}_render();`);if(k.click)lines.push(`lv_obj_add_flag(${k.rootName},LV_OBJ_FLAG_CLICKABLE);lv_obj_add_event_cb(${k.rootName},${k.click},LV_EVENT_CLICKED,NULL);`);if(k.detailsClick)lines.push(`lv_obj_t*${varName}_details=lv_btn_create(${k.rootName});lv_obj_set_pos(${varName}_details,${Math.max(100,Number(w)-112||188)},8);lv_obj_set_size(${varName}_details,88,30);lv_obj_add_event_cb(${varName}_details,${k.detailsClick},LV_EVENT_CLICKED,NULL);lv_obj_t*${varName}_details_label=lv_label_create(${varName}_details);lv_label_set_text(${varName}_details_label,"Details");lv_obj_center(${varName}_details_label);`);lines.push(``);break}

case 'PowerFlowCard':{const m=simulateForgeUIPowerFlow(normalizeForgeUIPowerFlowCard(child.props)),p=powerFlowExports.get(child.id);if(!p)break;const d=`${p.rootName}_data`;lines.push(`${p.rootName}=lv_obj_create(${parentVar});lv_obj_set_pos(${p.rootName},${x},${y});lv_obj_set_size(${p.rootName},${w},${h});lv_obj_set_style_bg_color(${p.rootName},lv_color_hex(${palette.surface}),LV_PART_MAIN);lv_obj_set_style_border_color(${p.rootName},lv_color_hex(${palette.surfaceBorder}),LV_PART_MAIN);lv_obj_set_style_radius(${p.rootName},${m.rounded?12:0},LV_PART_MAIN);lv_obj_clear_flag(${p.rootName},LV_OBJ_FLAG_SCROLLABLE);lv_obj_t*${varName}_title=lv_label_create(${p.rootName});lv_label_set_text(${varName}_title,"${esc(m.title)}");lv_obj_align(${varName}_title,LV_ALIGN_TOP_LEFT,0,0);${p.labelName}=lv_label_create(${p.rootName});lv_obj_set_pos(${p.labelName},0,34);lv_obj_set_size(${p.labelName},${Math.max(100,Number(w)-28||480)},${Math.max(100,Number(h)-62||298)});`);lines.push(`${d}.state=FG_POWER_FLOW_STATE_${m.state.toUpperCase()};${d}.nodes[FG_POWER_NODE_SOLAR].watts=${cFloatLiteral(m.solarPower)};${d}.nodes[FG_POWER_NODE_HOUSE_LOAD].watts=${cFloatLiteral(m.totalLoad)};${d}.nodes[FG_POWER_NODE_BATTERY].watts=${cFloatLiteral(m.batteryPower)};${d}.nodes[FG_POWER_NODE_GRID].watts=${cFloatLiteral(m.gridPower)};${d}.nodes[FG_POWER_NODE_GENERATOR].watts=${cFloatLiteral(m.generatorPower)};${d}.efficiency=${cFloatLiteral(m.efficiency)};${d}.energy_today=${cFloatLiteral(m.energyToday)};${d}.balance=${cFloatLiteral(m.systemBalance)};snprintf(${d}.status,sizeof(${d}.status),"%s","${escPrintfLiteral(m.statusText)}");${p.rootName}_render();`);if(p.click)lines.push(`lv_obj_add_flag(${p.rootName},LV_OBJ_FLAG_CLICKABLE);lv_obj_add_event_cb(${p.rootName},${p.click},LV_EVENT_CLICKED,NULL);`);lines.push(``);break}
...
export const generateForgeUILvglCode = (
  ...
  const tankCardExports = createTankCardExports(components, usedHookNames, userEventHooks)
  const networkCardExports = createNetworkCardExports(components, usedHookNames, userEventHooks)
  const deviceSummaryExports = createDeviceSummaryExports(components, usedHookNames, userEventHooks)
  const kpiCardExports = createKpiCardExports(components, usedHookNames, userEventHooks)
  const powerFlowExports = createPowerFlowExports(components, usedHookNames, userEventHooks)
...
const backgroundMode =
  ...
  if(tankCardExports.size>0){lines.push(`typedef enum { FG_TANK_STATE_UNKNOWN=0, FG_TANK_STATE_EMPTY, FG_TANK_STATE_FILLING, FG_TANK_STATE_DRAINING, FG_TANK_STATE_STABLE, FG_TANK_STATE_FULL, FG_TANK_STATE_LOW, FG_TANK_STATE_CRITICAL, FG_TANK_STATE_OVERFLOW, FG_TANK_STATE_FAULT, FG_TANK_STATE_DISCONNECTED } fg_tank_state_t;`);lines.push(`typedef struct { float percentage,volume,capacity,flow,low,critical,full,overflow; fg_tank_state_t state; char status[65]; } fg_tank_data_t;`)}
  tankCardExports.forEach(t=>{const d=`${t.rootName}_data`,render=`${t.rootName}_render`;lines.push(`static lv_obj_t*${t.rootName}=NULL;static lv_obj_t*${t.labelName}=NULL;static lv_obj_t*${t.barName}=NULL;static fg_tank_data_t ${d}={0};`);lines.push(`static void ${render}(void){if(${t.barName})lv_bar_set_value(${t.barName},(int32_t)${d}.percentage,LV_ANIM_OFF);if(${t.labelName})lv_label_set_text_fmt(${t.labelName},"%.0f%%\\n%.1f / %.1f L\\nFlow %.1f L/min\\n%s",(double)${d}.percentage,(double)${d}.volume,(double)${d}.capacity,(double)${d}.flow,${d}.status);}`);if(t.click)lines.push(`static void ${t.click}(lv_event_t*e){LV_UNUSED(e);${t.selected}();}`);if(t.runtimeEnabled){lines.push(`void ${t.setLevel}(float percentage){if(percentage<0)percentage=0;if(percentage>100)percentage=100;${d}.percentage=percentage;${render}();}`);lines.push(`void ${t.setVolume}(float volume){${d}.volume=volume;${render}();}`);lines.push(`void ${t.setCapacity}(float capacity){${d}.capacity=capacity;${render}();}`);lines.push(`void ${t.setState}(fg_tank_state_t state){${d}.state=state;${render}();}`);lines.push(`void ${t.setFlow}(float flow){${d}.flow=flow;${render}();}`);lines.push(`void ${t.setThresholds}(float low,float critical,float full){${d}.low=low;${d}.critical=critical;${d}.full=full;${render}();}`);lines.push(`void ${t.setStatus}(const char*status){if(!status)return;snprintf(${d}.status,sizeof(${d}.status),"%s",status);${render}();}`)}})
  if(networkCardExports.size>0||deviceSummaryExports.size>0)lines.push(`typedef enum { FG_NETWORK_STATE_UNKNOWN=0, FG_NETWORK_STATE_OFFLINE, FG_NETWORK_STATE_CONNECTING, FG_NETWORK_STATE_ONLINE, FG_NETWORK_STATE_DEGRADED, FG_NETWORK_STATE_RECONNECTING, FG_NETWORK_STATE_AUTHENTICATION_FAILED, FG_NETWORK_STATE_FAULT } fg_network_state_t;`)
  if(networkCardExports.size>0)lines.push(`typedef struct { fg_network_state_t state; int32_t rssi,latency; bool cloud,mqtt,internet,local_api; char ssid[65],ip[48],gateway[48],status[65]; } fg_network_data_t;`)
  networkCardExports.forEach(n=>{const d=`${n.rootName}_data`,render=`${n.rootName}_render`;lines.push(`static lv_obj_t*${n.rootName}=NULL;static lv_obj_t*${n.labelName}=NULL;static fg_network_data_t ${d}={0};`);lines.push(`static void ${render}(void){if(${n.labelName})lv_label_set_text_fmt(${n.labelName},"State %d\nSSID %s\nIP %s\nGateway %s\nRSSI %ld dBm  Latency %ld ms\nCloud %s  MQTT %s\n%s",(int)${d}.state,${d}.ssid,${d}.ip,${d}.gateway,(long)${d}.rssi,(long)${d}.latency,${d}.cloud?"Online":"Offline",${d}.mqtt?"Online":"Offline",${d}.status);}`);if(n.click)lines.push(`static void ${n.click}(lv_event_t*e){LV_UNUSED(e);${n.selected}();}`);if(n.runtimeEnabled){lines.push(`void ${n.setState}(fg_network_state_t state){${d}.state=state;${render}();}`);lines.push(`void ${n.setSsid}(const char*s){if(!s)return;snprintf(${d}.ssid,sizeof(${d}.ssid),"%s",s);${render}();}`);lines.push(`void ${n.setIp}(const char*s){if(!s)return;snprintf(${d}.ip,sizeof(${d}.ip),"%s",s);${render}();}`);lines.push(`void ${n.setGateway}(const char*s){if(!s)return;snprintf(${d}.gateway,sizeof(${d}.gateway),"%s",s);${render}();}`);lines.push(`void ${n.setRssi}(int32_t value){${d}.rssi=value;${render}();}`);lines.push(`void ${n.setLatency}(int32_t value){${d}.latency=value;${render}();}`);lines.push(`void ${n.setCloud}(bool connected){${d}.cloud=connected;${render}();}`);lines.push(`void ${n.setMqtt}(bool connected){${d}.mqtt=connected;${render}();}`);lines.push(`void ${n.setInternet}(bool available){${d}.internet=available;${render}();}`);lines.push(`void ${n.setLocalApi}(bool available){${d}.local_api=available;${render}();}`);lines.push(`void ${n.setStatus}(const char*s){if(!s)return;snprintf(${d}.status,sizeof(${d}.status),"%s",s);${render}();}`)}})
  lines.push(`static lv_obj_t * fg_application_page = NULL;`)
  if(deviceSummaryExports.size>0){lines.push(`typedef enum { FG_DEVICE_HEALTH_UNKNOWN=0, FG_DEVICE_HEALTH_INITIALISING, FG_DEVICE_HEALTH_HEALTHY, FG_DEVICE_HEALTH_ATTENTION, FG_DEVICE_HEALTH_WARNING, FG_DEVICE_HEALTH_DEGRADED, FG_DEVICE_HEALTH_FAULT, FG_DEVICE_HEALTH_MAINTENANCE, FG_DEVICE_HEALTH_OFFLINE, FG_DEVICE_HEALTH_DISCONNECTED } fg_device_health_t;`);lines.push(`typedef enum { FG_DEVICE_POWER_UNKNOWN=0, FG_DEVICE_POWER_MAINS, FG_DEVICE_POWER_BATTERY, FG_DEVICE_POWER_SOLAR, FG_DEVICE_POWER_EXTERNAL_DC, FG_DEVICE_POWER_USB, FG_DEVICE_POWER_BACKUP, FG_DEVICE_POWER_CHARGING, FG_DEVICE_POWER_SAVING, FG_DEVICE_POWER_CRITICAL_POWER, FG_DEVICE_POWER_OFFLINE } fg_device_power_t;`);lines.push(`typedef struct { fg_device_health_t health; fg_device_power_t power; fg_network_state_t network; uint32_t uptime,active,critical; uint64_t storage_free; float battery; bool storage,maintenance; char name[65],type[65],model[65],firmware[33],build[33],ip[48],maintenance_text[65],status[65]; } fg_device_summary_data_t;`)}
  deviceSummaryExports.forEach(s=>{const d=`${s.rootName}_data`,render=`${s.rootName}_render`;lines.push(`static lv_obj_t*${s.rootName}=NULL;static lv_obj_t*${s.labelName}=NULL;static fg_device_summary_data_t ${d}={0};`);lines.push(`static void ${render}(void){if(${s.labelName})lv_label_set_text_fmt(${s.labelName},"%s · %s\nHealth %d · Uptime %lu s\n%s · %s\nIP %s\nStorage %s · %llu free\nAlarms %lu · %lu critical\nPower %d · Battery %.0f%%\n%s%s%s",${d}.name,${d}.model,(int)${d}.health,(unsigned long)${d}.uptime,${d}.firmware,${d}.build,${d}.ip,${d}.storage?"Available":"Unavailable",(unsigned long long)${d}.storage_free,(unsigned long)${d}.active,(unsigned long)${d}.critical,(int)${d}.power,(double)${d}.battery,${d}.maintenance?"Maintenance: ":"",${d}.maintenance?${d}.maintenance_text:"",${d}.status);}`);if(s.click)lines.push(`static void ${s.click}(lv_event_t*e){LV_UNUSED(e);${s.selected}();}`);if(s.detailsClick)lines.push(`static void ${s.detailsClick}(lv_event_t*e){LV_UNUSED(e);${s.details}();}`);if(s.maintenanceClick)lines.push(`static void ${s.maintenanceClick}(lv_event_t*e){LV_UNUSED(e);${s.maintenance}();}`);if(s.alarmsClick)lines.push(`static void ${s.alarmsClick}(lv_event_t*e){LV_UNUSED(e);${s.alarms}();}`);if(s.runtimeEnabled){lines.push(`void ${s.setName}(const char*v){if(!v)return;snprintf(${d}.name,sizeof(${d}.name),"%s",v);${render}();}`);lines.push(`void ${s.setType}(const char*v){if(!v)return;snprintf(${d}.type,sizeof(${d}.type),"%s",v);${render}();}`);lines.push(`void ${s.setModel}(const char*v){if(!v)return;snprintf(${d}.model,sizeof(${d}.model),"%s",v);${render}();}`);lines.push(`void ${s.setFirmware}(const char*v){if(!v)return;snprintf(${d}.firmware,sizeof(${d}.firmware),"%s",v);${render}();}`);lines.push(`void ${s.setBuild}(const char*v){if(!v)return;snprintf(${d}.build,sizeof(${d}.build),"%s",v);${render}();}`);lines.push(`void ${s.setHealth}(fg_device_health_t v){${d}.health=v;${render}();}`);lines.push(`void ${s.setUptime}(uint32_t v){${d}.uptime=v;${render}();}`);lines.push(`void ${s.setNetwork}(fg_network_state_t v){${d}.network=v;${render}();}`);lines.push(`void ${s.setIp}(const char*v){if(!v)return;snprintf(${d}.ip,sizeof(${d}.ip),"%s",v);${render}();}`);lines.push(`void ${s.setStorage}(bool available,uint64_t free_bytes){${d}.storage=available;${d}.storage_free=free_bytes;${render}();}`);lines.push(`void ${s.setAlarms}(uint32_t active,uint32_t critical){${d}.active=active;${d}.critical=critical;${render}();}`);lines.push(`void ${s.setPower}(fg_device_power_t v){${d}.power=v;${render}();}`);lines.push(`void ${s.setBattery}(float v){if(v<0)v=0;if(v>100)v=100;${d}.battery=v;${render}();}`);lines.push(`void ${s.setMaintenance}(bool required,const char*message){${d}.maintenance=required;if(message)snprintf(${d}.maintenance_text,sizeof(${d}.maintenance_text),"%s",message);${render}();}`);lines.push(`void ${s.setStatus}(const char*v){if(!v)return;snprintf(${d}.status,sizeof(${d}.status),"%s",v);${render}();}`)}})
  if(kpiCardExports.size)lines.push(`typedef struct { float value,progress,delta,target; uint32_t colour; char units[17],status[65],updated[65]; } fg_kpi_data_t;`)
  kpiCardExports.forEach(k=>{const d=`${k.rootName}_data`,render=`${k.rootName}_render`;lines.push(`static lv_obj_t*${k.rootName}=NULL;static lv_obj_t*${k.labelName}=NULL;static lv_obj_t*${k.barName}=NULL;static fg_kpi_data_t ${d}={0};`);lines.push(`static void ${render}(void){if(${k.labelName}){lv_label_set_text_fmt(${k.labelName},"%.1f %s\n%s\nDelta %.1f · Target %.1f\n%s",(double)${d}.value,${d}.units,${d}.status,(double)${d}.delta,(double)${d}.target,${d}.updated);lv_obj_set_style_text_color(${k.labelName},lv_color_hex(${d}.colour),LV_PART_MAIN);}if(${k.barName})lv_bar_set_value(${k.barName},(int32_t)${d}.progress,LV_ANIM_OFF);}`);if(k.click)lines.push(`static void ${k.click}(lv_event_t*e){LV_UNUSED(e);${k.selected}();}`);if(k.detailsClick)lines.push(`static void ${k.detailsClick}(lv_event_t*e){LV_UNUSED(e);${k.details}();}`);if(k.runtimeEnabled){lines.push(`void ${k.setValue}(float v){${d}.value=v;${render}();}`);lines.push(`void ${k.setUnits}(const char*v){if(!v)return;snprintf(${d}.units,sizeof(${d}.units),"%s",v);${render}();}`);lines.push(`void ${k.setStatus}(const char*v){if(!v)return;snprintf(${d}.status,sizeof(${d}.status),"%s",v);${render}();}`);lines.push(`void ${k.setColour}(uint32_t rgb){${d}.colour=rgb&0xFFFFFFu;${render}();}`);lines.push(`void ${k.setProgress}(float v){if(v<0)v=0;if(v>100)v=100;${d}.progress=v;${render}();}`);lines.push(`void ${k.setDelta}(float v){${d}.delta=v;${render}();}`);lines.push(`void ${k.setTarget}(float v){${d}.target=v;${render}();}`);lines.push(`void ${k.setUpdated}(const char*v){if(!v)return;snprintf(${d}.updated,sizeof(${d}.updated),"%s",v);${render}();}`)}})
  if(powerFlowExports.size){lines.push(`typedef enum { FG_POWER_FLOW_STATE_UNKNOWN=0, FG_POWER_FLOW_STATE_IDLE, FG_POWER_FLOW_STATE_ACTIVE, FG_POWER_FLOW_STATE_IMPORTING, FG_POWER_FLOW_STATE_EXPORTING, FG_POWER_FLOW_STATE_CHARGING, FG_POWER_FLOW_STATE_DISCHARGING, FG_POWER_FLOW_STATE_STANDBY, FG_POWER_FLOW_STATE_OFFLINE, FG_POWER_FLOW_STATE_FAULT, FG_POWER_FLOW_STATE_DISABLED } fg_power_flow_state_t;`);lines.push(`typedef enum { FG_POWER_NODE_SOLAR=0, FG_POWER_NODE_GRID, FG_POWER_NODE_BATTERY, FG_POWER_NODE_GENERATOR, FG_POWER_NODE_WIND, FG_POWER_NODE_CHARGER, FG_POWER_NODE_INVERTER, FG_POWER_NODE_HOUSE_LOAD, FG_POWER_NODE_CRITICAL_LOAD, FG_POWER_NODE_EV_CHARGER, FG_POWER_NODE_MOTOR_LOAD, FG_POWER_NODE_AUXILIARY_LOAD, FG_POWER_NODE_SHORE_POWER, FG_POWER_NODE_EXTERNAL_DC, FG_POWER_NODE_GENERIC_SOURCE, FG_POWER_NODE_GENERIC_STORAGE, FG_POWER_NODE_GENERIC_LOAD, FG_POWER_NODE_COUNT } fg_power_node_t;`);lines.push(`typedef struct { float watts; bool enabled,online,faulted; char label[33]; } fg_power_node_data_t;typedef struct { fg_power_node_t source,destination; float watts; bool enabled,active; } fg_power_connection_data_t;typedef struct { fg_power_flow_state_t state; fg_power_node_data_t nodes[FG_POWER_NODE_COUNT]; fg_power_connection_data_t connections[12]; float efficiency,energy_today,balance; char status[65]; } fg_power_flow_data_t;`)}
  powerFlowExports.forEach(p=>{const d=`${p.rootName}_data`,render=`${p.rootName}_render`;lines.push(`static lv_obj_t*${p.rootName}=NULL;static lv_obj_t*${p.labelName}=NULL;static fg_power_flow_data_t ${d}={0};`);lines.push(`static void ${render}(void){if(${p.labelName})lv_label_set_text_fmt(${p.labelName},"Solar %.2f kW  Grid %.2f kW\nBattery %.2f kW  Load %.2f kW\nGenerator %.2f kW\nEfficiency %.1f%%  Energy %.1f kWh\nBalance %.2f kW\n%s",(double)${d}.nodes[FG_POWER_NODE_SOLAR].watts/1000.0,(double)${d}.nodes[FG_POWER_NODE_GRID].watts/1000.0,(double)${d}.nodes[FG_POWER_NODE_BATTERY].watts/1000.0,(double)${d}.nodes[FG_POWER_NODE_HOUSE_LOAD].watts/1000.0,(double)${d}.nodes[FG_POWER_NODE_GENERATOR].watts/1000.0,(double)${d}.efficiency,(double)${d}.energy_today,(double)${d}.balance/1000.0,${d}.status);}`);if(p.click)lines.push(`static void ${p.click}(lv_event_t*e){LV_UNUSED(e);${p.selected}();}`);if(p.runtimeEnabled){const setter=(name:string,node:string)=>lines.push(`void ${name}(float watts){${d}.nodes[${node}].watts=watts;${render}();}`);setter(p.setSolar,'FG_POWER_NODE_SOLAR');setter(p.setLoad,'FG_POWER_NODE_HOUSE_LOAD');setter(p.setBattery,'FG_POWER_NODE_BATTERY');setter(p.setGrid,'FG_POWER_NODE_GRID');setter(p.setGenerator,'FG_POWER_NODE_GENERATOR');lines.push(`void ${p.setEfficiency}(float percent){if(percent<0)percent=0;if(percent>100)percent=100;${d}.efficiency=percent;${render}();}`);lines.push(`void ${p.setEnergy}(float kwh){${d}.energy_today=kwh;${render}();}`);lines.push(`void ${p.setState}(fg_power_flow_state_t state){${d}.state=state;${render}();}`);lines.push(`void ${p.setOnline}(fg_power_node_t node,bool online){if(node>=FG_POWER_NODE_COUNT)return;${d}.nodes[node].online=online;${render}();}`);lines.push(`void ${p.setFault}(fg_power_node_t node,bool faulted){if(node>=FG_POWER_NODE_COUNT)return;${d}.nodes[node].faulted=faulted;${render}();}`);lines.push(`void ${p.setStatus}(const char*status){if(!status)return;snprintf(${d}.status,sizeof(${d}.status),"%s",status);${render}();}`);lines.push(`void ${p.setNode}(fg_power_node_t node,float watts){if(node>=FG_POWER_NODE_COUNT)return;${d}.nodes[node].watts=watts;${render}();}`);lines.push(`void ${p.setConnection}(fg_power_node_t source,fg_power_node_t destination,float watts){for(uint32_t i=0;i<12u;i++){if(${d}.connections[i].source==source&&${d}.connections[i].destination==destination){${d}.connections[i].watts=watts;${d}.connections[i].active=true;${render}();return;}}}`)}})
...
lines.push(`}`)
  ...
  const fiRuntime = generateFiRuntimeFiles(
    ...
    publicApiDeclarations: Array.from(binaryOutputExports.values())
      ...
      ])).concat(tankCardExports.size?['typedef enum { FG_TANK_STATE_UNKNOWN=0, FG_TANK_STATE_EMPTY, FG_TANK_STATE_FILLING, FG_TANK_STATE_DRAINING, FG_TANK_STATE_STABLE, FG_TANK_STATE_FULL, FG_TANK_STATE_LOW, FG_TANK_STATE_CRITICAL, FG_TANK_STATE_OVERFLOW, FG_TANK_STATE_FAULT, FG_TANK_STATE_DISCONNECTED } fg_tank_state_t;']:[]).concat(Array.from(tankCardExports.values()).filter(t=>t.runtimeEnabled).flatMap(t=>[`void ${t.setLevel}(float percentage);`,`void ${t.setVolume}(float volume);`,`void ${t.setCapacity}(float capacity);`,`void ${t.setState}(fg_tank_state_t state);`,`void ${t.setFlow}(float flow);`,`void ${t.setThresholds}(float low, float critical, float full);`,`void ${t.setStatus}(const char * status);`])).concat((networkCardExports.size||deviceSummaryExports.size)?['typedef enum { FG_NETWORK_STATE_UNKNOWN=0, FG_NETWORK_STATE_OFFLINE, FG_NETWORK_STATE_CONNECTING, FG_NETWORK_STATE_ONLINE, FG_NETWORK_STATE_DEGRADED, FG_NETWORK_STATE_RECONNECTING, FG_NETWORK_STATE_AUTHENTICATION_FAILED, FG_NETWORK_STATE_FAULT } fg_network_state_t;']:[]).concat(Array.from(networkCardExports.values()).filter(n=>n.runtimeEnabled).flatMap(n=>[`void ${n.setState}(fg_network_state_t state);`,`void ${n.setSsid}(const char * ssid);`,`void ${n.setIp}(const char * ip);`,`void ${n.setGateway}(const char * gateway);`,`void ${n.setRssi}(int32_t rssi);`,`void ${n.setLatency}(int32_t latency_ms);`,`void ${n.setCloud}(bool connected);`,`void ${n.setMqtt}(bool connected);`,`void ${n.setInternet}(bool available);`,`void ${n.setLocalApi}(bool available);`,`void ${n.setStatus}(const char * status);`])).concat(deviceSummaryExports.size?['typedef enum { FG_DEVICE_HEALTH_UNKNOWN=0, FG_DEVICE_HEALTH_INITIALISING, FG_DEVICE_HEALTH_HEALTHY, FG_DEVICE_HEALTH_ATTENTION, FG_DEVICE_HEALTH_WARNING, FG_DEVICE_HEALTH_DEGRADED, FG_DEVICE_HEALTH_FAULT, FG_DEVICE_HEALTH_MAINTENANCE, FG_DEVICE_HEALTH_OFFLINE, FG_DEVICE_HEALTH_DISCONNECTED } fg_device_health_t;','typedef enum { FG_DEVICE_POWER_UNKNOWN=0, FG_DEVICE_POWER_MAINS, FG_DEVICE_POWER_BATTERY, FG_DEVICE_POWER_SOLAR, FG_DEVICE_POWER_EXTERNAL_DC, FG_DEVICE_POWER_USB, FG_DEVICE_POWER_BACKUP, FG_DEVICE_POWER_CHARGING, FG_DEVICE_POWER_SAVING, FG_DEVICE_POWER_CRITICAL_POWER, FG_DEVICE_POWER_OFFLINE } fg_device_power_t;']:[]).concat(Array.from(deviceSummaryExports.values()).filter(s=>s.runtimeEnabled).flatMap(s=>[`void ${s.setName}(const char * name);`,`void ${s.setType}(const char * type);`,`void ${s.setModel}(const char * model);`,`void ${s.setFirmware}(const char * version);`,`void ${s.setBuild}(const char * build);`,`void ${s.setHealth}(fg_device_health_t health);`,`void ${s.setUptime}(uint32_t seconds);`,`void ${s.setNetwork}(fg_network_state_t state);`,`void ${s.setIp}(const char * ip);`,`void ${s.setStorage}(bool available, uint64_t free_bytes);`,`void ${s.setAlarms}(uint32_t active, uint32_t critical);`,`void ${s.setPower}(fg_device_power_t source);`,`void ${s.setBattery}(float percentage);`,`void ${s.setMaintenance}(bool required, const char * message);`,`void ${s.setStatus}(const char * status);`])).concat(Array.from(barExports.values()).map(
        barExport => `void ${barExport.apiName}(int32_t value);`,
      )).concat(Array.from(kpiCardExports.values()).filter(k=>k.runtimeEnabled).flatMap(k=>[`void ${k.setValue}(float value);`,`void ${k.setUnits}(const char * units);`,`void ${k.setStatus}(const char * status);`,`void ${k.setColour}(uint32_t rgb);`,`void ${k.setProgress}(float percent);`,`void ${k.setDelta}(float delta);`,`void ${k.setTarget}(float target);`,`void ${k.setUpdated}(const char * text);`])).concat(powerFlowExports.size?['typedef enum { FG_POWER_FLOW_STATE_UNKNOWN=0, FG_POWER_FLOW_STATE_IDLE, FG_POWER_FLOW_STATE_ACTIVE, FG_POWER_FLOW_STATE_IMPORTING, FG_POWER_FLOW_STATE_EXPORTING, FG_POWER_FLOW_STATE_CHARGING, FG_POWER_FLOW_STATE_DISCHARGING, FG_POWER_FLOW_STATE_STANDBY, FG_POWER_FLOW_STATE_OFFLINE, FG_POWER_FLOW_STATE_FAULT, FG_POWER_FLOW_STATE_DISABLED } fg_power_flow_state_t;','typedef enum { FG_POWER_NODE_SOLAR=0, FG_POWER_NODE_GRID, FG_POWER_NODE_BATTERY, FG_POWER_NODE_GENERATOR, FG_POWER_NODE_WIND, FG_POWER_NODE_CHARGER, FG_POWER_NODE_INVERTER, FG_POWER_NODE_HOUSE_LOAD, FG_POWER_NODE_CRITICAL_LOAD, FG_POWER_NODE_EV_CHARGER, FG_POWER_NODE_MOTOR_LOAD, FG_POWER_NODE_AUXILIARY_LOAD, FG_POWER_NODE_SHORE_POWER, FG_POWER_NODE_EXTERNAL_DC, FG_POWER_NODE_GENERIC_SOURCE, FG_POWER_NODE_GENERIC_STORAGE, FG_POWER_NODE_GENERIC_LOAD, FG_POWER_NODE_COUNT } fg_power_node_t;']:[]).concat(Array.from(powerFlowExports.values()).filter(p=>p.runtimeEnabled).flatMap(p=>[`void ${p.setSolar}(float watts);`,`void ${p.setLoad}(float watts);`,`void ${p.setBattery}(float watts);`,`void ${p.setGrid}(float watts);`,`void ${p.setGenerator}(float watts);`,`void ${p.setEfficiency}(float percent);`,`void ${p.setEnergy}(float kwh);`,`void ${p.setState}(fg_power_flow_state_t state);`,`void ${p.setOnline}(fg_power_node_t node, bool online);`,`void ${p.setFault}(fg_power_node_t node, bool faulted);`,`void ${p.setStatus}(const char * status);`,`void ${p.setNode}(fg_power_node_t node, float watts);`,`void ${p.setConnection}(fg_power_node_t source, fg_power_node_t destination, float watts);`])).concat(Array.from(progressExports.values()).map(
