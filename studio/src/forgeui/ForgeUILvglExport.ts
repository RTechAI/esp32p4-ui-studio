...
lines.push(`}`)
  ...
  const fiRuntime = generateFiRuntimeFiles(
    ...
    publicApiDeclarations: Array.from(binaryOutputExports.values())
      ...
      )).concat(Array.from(kpiCardExports.values()).filter(k=>k.runtimeEnabled).flatMap(k=>[`void ${k.setValue}(float value);`,`void ${k.setUnits}(const char * units);`,`void ${k.setStatus}(const char * status);`,`void ${k.setColour}(uint32_t rgb);`,`void ${k.setProgress}(float percent);`,`void ${k.setDelta}(float delta);`,`void ${k.setTarget}(float target);`,`void ${k.setUpdated}(const char * text);`])).concat(Array.from(progressExports.values()).map(
