...
const backgroundMode =
  ...
  if(networkCardExports.size>0){lines.push(`typedef enum { FG_NETWORK_STATE_UNKNOWN=0, FG_NETWORK_STATE_OFFLINE, FG_NETWORK_STATE_CONNECTING, FG_NETWORK_STATE_ONLINE, FG_NETWORK_STATE_DEGRADED, FG_NETWORK_STATE_RECONNECTING, FG_NETWORK_STATE_AUTHENTICATION_FAILED, FG_NETWORK_STATE_FAULT } fg_network_state_t;`);lines.push(`typedef struct { fg_network_state_t state; int32_t rssi,latency; bool cloud,mqtt,internet,local_api; char ssid[65],ip[48],gateway[48],status[65]; } fg_network_data_t;`)}
