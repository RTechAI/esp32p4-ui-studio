...
const backgroundMode =
  ...
  lines.push(`        lv_obj_add_flag(fg_system_wifi_keyboard, LV_OBJ_FLAG_HIDDEN);`)
  lines.push(`    }`)
  lines.push(`    if (lv_keyboard_get_textarea(fg_system_wifi_keyboard) != fg_system_wifi_password_input) {`)
  lines.push(`        lv_keyboard_set_textarea(fg_system_wifi_keyboard, fg_system_wifi_password_input);`)
  lines.push(`    }`)
...
lines.push(`}`)
  ...
  lines.push(`        lv_obj_set_style_border_width(fg_system_wifi_network_rows[i], i == fg_system_wifi_selected ? 3 : 1, 0);`)
  lines.push(`    }`)
  lines.push(`}`)
  lines.push(``)
