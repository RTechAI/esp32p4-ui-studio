...
const backgroundMode =
  ...
  lines.push(`static lv_obj_t * fg_system_wifi_network_list = NULL;`)
  ...
  lines.push(`static lv_obj_t * fg_system_wifi_connect_button = NULL;`)
  lines.push(`static char fg_system_wifi_network_cache[512] = "";`)
  ...
  lines.push(`{`)
  lines.push(`    if (!fg_application_page || !fg_system_launcher_page || !fg_system_brightness_page || !fg_system_wifi_page) return;`)
...
lines.push(`}`)
  ...
  lines.push(`    }`)
  lines.push(``)
  lines.push(`    if (!fg_system_wifi_page) return;`)
  lines.push(``)
  lines.push(`    const char * state_text = "OFF";`)
  lines.push(`    switch (fg_wifi_state()) {`)
  lines.push(`        case FG_WIFI_STATE_INIT: state_text = "Initializing"; break;`)
  ...
  lines.push(`        case FG_WIFI_STATE_ERROR: state_text = "Error"; break;`)
  lines.push(`        case FG_WIFI_STATE_OFF:`)
  lines.push(`        default: break;`)
  lines.push(`    }`)
  lines.push(`    bool scanning = fg_wifi_scan_in_progress();`)
  lines.push(`    if (fg_system_wifi_state_label) lv_label_set_text(fg_system_wifi_state_label, scanning ? "Scanning" : state_text);`)
  lines.push(`    if (fg_system_wifi_ssid_label) lv_label_set_text_fmt(fg_system_wifi_ssid_label, "Network: %s", fg_wifi_ssid_text());`)
  lines.push(`    if (fg_system_wifi_ip_label) lv_label_set_text_fmt(fg_system_wifi_ip_label, "IP: %s", fg_wifi_ip_text());`)
  lines.push(`    if (fg_system_wifi_rssi_label) {`)
  lines.push(`        int rssi = fg_wifi_rssi();`)
  lines.push(`        if (rssi == 0) lv_label_set_text(fg_system_wifi_rssi_label, "Signal: unavailable");`)
  lines.push(`        else lv_label_set_text_fmt(fg_system_wifi_rssi_label, "Signal: %d dBm", rssi);`)
  lines.push(`    }`)
  lines.push(`    if (fg_system_wifi_raw_label) lv_label_set_text_fmt(fg_system_wifi_raw_label, "Status: %s", fg_wifi_status_text());`)
  lines.push(`    if (fg_system_wifi_scan_label) lv_label_set_text(fg_system_wifi_scan_label, scanning ? "Scanning for nearby networks..." : "Available Networks");`)
  lines.push(``)
  lines.push(`    if (fg_system_wifi_scan_button) {`)
  lines.push(`        if (scanning) lv_obj_add_state(fg_system_wifi_scan_button, LV_STATE_DISABLED);`)
  lines.push(`        else lv_obj_clear_state(fg_system_wifi_scan_button, LV_STATE_DISABLED);`)
  lines.push(`    }`)
  lines.push(`    if (fg_system_wifi_disconnect_button) {`)
  lines.push(`        fg_wifi_state_t state = fg_wifi_state();`)
  lines.push(`        if (state == FG_WIFI_STATE_CONNECTED || state == FG_WIFI_STATE_CONNECTING) lv_obj_clear_state(fg_system_wifi_disconnect_button, LV_STATE_DISABLED);`)
  lines.push(`        else lv_obj_add_state(fg_system_wifi_disconnect_button, LV_STATE_DISABLED);`)
  lines.push(`    }`)
  lines.push(``)
  lines.push(`    char ssids[12][33] = {{0}};`)
  lines.push(`    int count = fg_wifi_get_scan_results(ssids, 12);`)
  lines.push(`    char network_text[512] = "";`)
  lines.push(`    size_t used = 0;`)
  lines.push(`    for (int i = 0; i < count && used < sizeof(network_text); i++) {`)
  lines.push(`        int written = snprintf(network_text + used, sizeof(network_text) - used, "%s%s", i ? "\\n" : "", ssids[i]);`)
  lines.push(`        if (written < 0 || (size_t)written >= sizeof(network_text) - used) break;`)
  lines.push(`        used += (size_t)written;`)
  lines.push(`    }`)
  lines.push(`    if (count == 0) snprintf(network_text, sizeof(network_text), "%s", scanning ? "Scanning..." : "No networks found. Tap Scan Networks.");`)
  lines.push(`    if (fg_system_wifi_network_list && strcmp(network_text, fg_system_wifi_network_cache) != 0) {`)
  lines.push(`        snprintf(fg_system_wifi_network_cache, sizeof(fg_system_wifi_network_cache), "%s", network_text);`)
  lines.push(`        lv_label_set_text(fg_system_wifi_network_list, fg_system_wifi_network_cache);`)
  lines.push(`    }`)
  lines.push(`}`)
  lines.push(``)
...
lines.push(`    lv_obj_set_size(wifi_status_panel, 440, 282);`)
...
lines.push(`    lv_obj_set_pos(fg_system_wifi_ssid_label, 14, 64);`)
lines.push(`    lv_obj_set_width(fg_system_wifi_ssid_label, 390);`)
...
lines.push(`    lv_obj_set_pos(fg_system_wifi_ip_label, 14, 104);`)
lines.push(`    fg_system_wifi_rssi_label = lv_label_create(wifi_status_panel);`)
lines.push(`    lv_obj_set_pos(fg_system_wifi_rssi_label, 14, 144);`)
lines.push(`    fg_system_wifi_raw_label = lv_label_create(wifi_status_panel);`)
lines.push(`    lv_obj_set_pos(fg_system_wifi_raw_label, 14, 184);`)
lines.push(`    lv_obj_set_width(fg_system_wifi_raw_label, 390);`)
lines.push(`    lv_label_set_long_mode(fg_system_wifi_raw_label, LV_LABEL_LONG_DOT);`)
lines.push(``)
lines.push(`    fg_system_wifi_scan_button = fg_system_create_button(fg_system_wifi_page, LV_SYMBOL_REFRESH "  Scan Networks", 28, 394, 210, 62);`)
lines.push(`    lv_obj_add_event_cb(fg_system_wifi_scan_button, fg_system_wifi_scan_cb, LV_EVENT_CLICKED, NULL);`)
lines.push(`    fg_system_wifi_disconnect_button = fg_system_create_button(fg_system_wifi_page, "Disconnect", 248, 394, 220, 62);`)
lines.push(`    lv_obj_add_event_cb(fg_system_wifi_disconnect_button, fg_system_wifi_disconnect_cb, LV_EVENT_CLICKED, NULL);`)
lines.push(`    fg_system_wifi_connect_button = fg_system_create_button(fg_system_wifi_page, "Connect", 28, 468, 440, 58);`)
lines.push(`    lv_obj_add_state(fg_system_wifi_connect_button, LV_STATE_DISABLED);`)
lines.push(`    lv_obj_t * wifi_connect_help = lv_label_create(fg_system_wifi_page);`)
lines.push(`    lv_label_set_text(wifi_connect_help, "Network selection and password support are coming later.");`)
lines.push(`    lv_obj_set_pos(wifi_connect_help, 42, 540);`)
lines.push(`    lv_obj_set_style_text_opa(wifi_connect_help, LV_OPA_60, 0);`)
lines.push(``)
...
lines.push(`    fg_system_wifi_network_list = lv_label_create(fg_system_wifi_network_container);`)
lines.push(`    lv_obj_set_pos(fg_system_wifi_network_list, 8, 8);`)
lines.push(`    lv_obj_set_width(fg_system_wifi_network_list, 462);`)
lines.push(`    lv_label_set_long_mode(fg_system_wifi_network_list, LV_LABEL_LONG_WRAP);`)
lines.push(`    lv_label_set_text(fg_system_wifi_network_list, "No networks found. Tap Scan Networks.");`)
lines.push(`    lv_obj_add_flag(fg_system_wifi_page, LV_OBJ_FLAG_HIDDEN);`)
lines.push(``)
