...
export const ForgeUIBatteryCardPreview = ({ component, palette }: { component: IComponent; palette: ForgePreviewPalette }) => {
  ...
  return <Flex data-testid="forgeui-battery-card" pointerEvents="none" direction="column" width="100%" height="100%" overflow="hidden" bg={theme.surface} color={theme.textPrimary} border={`1px solid ${theme.surfaceBorder}`} borderRadius="10px" p={model.compactMode ? '8px' : '12px'} gap={model.compactMode ? '5px' : '8px'}>
    <Flex justify="space-between" align="center" gap="8px">
      <Text fontWeight="700" fontSize={model.compactMode ? '12px' : '14px'} noOfLines={1}>{model.title}</Text>
      <Flex align="center" gap="6px" flexShrink={0}>
        <BatteryIcon value={model.percentage} colour={colour} theme={theme} />
        <Text color={colour} fontWeight="700" fontSize="10px">{model.showChargingIcon && model.charging ? '⚡ CHARGING' : model.health.toUpperCase()}</Text>
      </Flex>
    </Flex>
    <Flex align="baseline" gap="5px">{model.showPercentage && <><Text fontSize={model.compactMode ? '26px' : '34px'} lineHeight="1" fontWeight="800">{Number(model.percentage.toFixed(1))}</Text><Text color={theme.textSecondary}>{model.units}</Text></>}</Flex>
    <Progress value={model.percentage} size="sm" colorScheme={model.percentage <= model.criticalThreshold ? 'red' : model.percentage <= model.lowThreshold ? 'yellow' : 'green'} borderRadius="4px" bg={theme.surfaceSecondary} />
    <Flex flexWrap="wrap" gap={model.compactMode ? '5px' : '8px'}>
...
const Metric = ({ label, value, theme }: { label: string; value: string; theme: ReturnType<typeof resolveForgeSemanticPalette> }) => <Box flex="1" minWidth="62px" bg={theme.surfaceSecondary} borderRadius="5px" px="6px" py="4px"><Text fontSize="7px" color={theme.textSecondary}>{label}</Text><Text fontSize="10px" fontWeight="700" noOfLines={1}>{value}</Text></Box>
...
const BatteryIcon = ({ value, colour, theme }: { value: number; colour: string; theme: ReturnType<typeof resolveForgeSemanticPalette> }) => (
  <Box position="relative" width="24px" height="12px" border={`1px solid ${theme.textSecondary}`} borderRadius="3px" flexShrink={0}>
    <Box position="absolute" left="2px" top="2px" height="6px" width={`${Math.max(0, Math.min(100, value)) * 0.18}px`} bg={colour} borderRadius="2px" />
