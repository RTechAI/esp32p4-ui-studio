...
const sensorTileFormLabelProps = { fontSize: 'xs', color: INSPECTOR_PROPERTY_TEXT_COLOR, opacity: 1 }
const sensorTileCheckboxProps = {
...
export const SensorTilePanel = () => {
  ...
  const input = (label: string, name: string, value: string) => <FormControl><FormLabel {...sensorTileFormLabelProps}>{label}</FormLabel><Input size="sm" value={value} onChange={e => setValue(name, e.target.value)} /></FormControl>
  const number = (label: string, name: string, value: number) => <FormControl><FormLabel {...sensorTileFormLabelProps}>{label}</FormLabel><NumberInput size="sm" value={value} onChange={(_, n) => setValue(name, Number.isFinite(n) ? n : 0)}><NumberInputField /></NumberInput></FormControl>
  return <Stack spacing={3} color={INSPECTOR_PROPERTY_TEXT_COLOR}><Text fontSize="sm" fontWeight="bold">ForgeUI Native Sensor Tile</Text>
    <FormControl><FormLabel {...sensorTileFormLabelProps}>Sensor type</FormLabel><Select size="sm" value={model.sensorType} onChange={e => setValue('sensorType', e.target.value)}>{['temperature','pressure','humidity','voltage','current','power','energy','rpm','frequency','generic'].map(type => <option key={type} value={type}>{type.charAt(0).toUpperCase()+type.slice(1)}</option>)}</Select></FormControl>
    ...
    <FormControl><FormLabel {...sensorTileFormLabelProps}>Status</FormLabel><Select size="sm" value={model.status} onChange={e => setValue('status', e.target.value)}>{['normal','warning','critical','offline'].map(v => <option key={v}>{v}</option>)}</Select></FormControl>
    {input('Status text','statusText',model.statusText)}
    <FormControl><FormLabel {...sensorTileFormLabelProps}>Trend</FormLabel><Select size="sm" value={model.trend} onChange={e => setValue('trend', e.target.value)}>{['rising','falling','stable'].map(v => <option key={v}>{v}</option>)}</Select></FormControl>
    {input('Timestamp','timestamp',model.timestamp)}
    <FormControl><FormLabel {...sensorTileFormLabelProps}>Accent colour</FormLabel><Input size="sm" type="color" value={model.accentColor || '#14B8A6'} onChange={e => setValue('accentColor', e.target.value)} /></FormControl>
    {number('Padding','padding',model.padding)}
    <Text fontSize="xs" fontWeight="bold" color={INSPECTOR_PROPERTY_TEXT_COLOR}>Engineering ranges</Text>
    ...
    <Checkbox {...sensorTileCheckboxProps} isChecked={model.autoColour} onChange={e => setValue('autoColour', e.target.checked)}>Auto colour from ranges</Checkbox>
    <Checkbox {...sensorTileCheckboxProps} isChecked={model.showTrend} onChange={e => setValue('showTrend', e.target.checked)}>Show trend</Checkbox>
    <Checkbox {...sensorTileCheckboxProps} isChecked={model.showProgress} onChange={e => setValue('showProgress', e.target.checked)}>Show progress</Checkbox>
    <Checkbox {...sensorTileCheckboxProps} isChecked={model.showTimestamp} onChange={e => setValue('showTimestamp', e.target.checked)}>Show timestamp</Checkbox>
    <Checkbox {...sensorTileCheckboxProps} isChecked={model.enableClick} onChange={e => setValue('enableClick', e.target.checked)}>Generate click UserEvent</Checkbox>
  </Stack>
}
