...
TabView retains its LVGL object, selected index and tab count. Touch and programmatic changes use one transition helper. Programmatic updates call `lv_tabview_set_active(..., LV_ANIM_OFF)` and `LV_EVENT_VALUE_CHANGED` touch handling reads `lv_tabview_get_tab_active()`. Generated identifiers are collision-safe and creation is silent.

### Tileview

Tileview retains its LVGL object, explicit tile map, selected row and column, and retained grid dimensions. Explicit coordinate mapping connects coordinates to tile objects. Touch and programmatic changes use one transition helper. Programmatic updates call `lv_tileview_set_tile(..., LV_ANIM_OFF)` and `LV_EVENT_VALUE_CHANGED` touch handling reads `lv_tileview_get_tile_active()`. Generated identifiers are collision-safe and creation is silent.
...
Line generates an LVGL `lv_line` with two points. It owns no runtime semantics, public API or hook; omission of an API is intentional.
...
- **Text Value:** generated firmware calls `lv_label_set_text(label, ...)`. There is no runtime transition, API or hook.
- **Heading Text:** generated firmware calls `lv_label_set_text(heading, ...)`. There is no runtime transition, API or hook.
...
- Tileview retained runtime and explicit tile-map generation
...
- Tileview retained runtime, coordinate map, setter and touch callback
...
| Tileview coordinate is wrong | `ForgeUILvglExport.ts` Tileview runtime | retained map/dimensions, shared transition helper, `lv_tileview_set_tile()` and `lv_tileview_get_tile_active()` |
